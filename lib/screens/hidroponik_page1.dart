...
class HidroponikPage1 extends StatefulWidget {
  @override
  _HidroponikPage1State createState() => _HidroponikPage1State();
}

class _HidroponikPage1State extends State<HidroponikPage1> {
  ...
  Future<void> fetchDataFromAPI() async {
    final url = Uri.parse('https://usriyusron.my.id/api/fetchAPI.php'); 
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['sensors'] != null) {
          setState(() {
            ...
            _processChartData(jsonResponse); //proses data untuk grafik
            isLoading = false; 
          });
        } else {
          setState(() {
  ...
  void _processChartData(Map<String, dynamic> jsonResponse) {
    ...
    jsonResponse['sensors'].forEach((key, value) {
      try {
        ...
        // data suhu dan kekeruhan dalam list
    ...
    if (suhuTempData.length > 20) {
      suhuTempData = suhuTempData.take(20).toList(); 
    }

    if (kekeruhanTempData.length > 20) {
      kekeruhanTempData = kekeruhanTempData.take(20).toList(); 
    }
  ...
  child: Row(
    children: [
      Icon(
        Icons.local_florist, // Ikon yang sesuai, misalnya ikon tetesan air
